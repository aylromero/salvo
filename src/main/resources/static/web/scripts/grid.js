    ...
    for(let i = 0; i < size; i++){
        ...
        for(let j = 0; j < size; j++){
            ...
            if(i > 0 && j > 0){
                ...
                if(element.id == 'grid-salvo')
                    cell.classList.add('shootEnabled');
            }
...
function checkShipsOnTheGrid(){
    
    let children= Array.from(document.getElementById('dock').children)
    let typeShips= ["carrier","battleship","submarine","destroyer","patrol_boat"]

    for(let i=0; i<children.length; i++){
        if(typeShips.includes(children[i].id)){
            document.getElementById('btn-AddShip').disabled= true;
        }else{
            document.getElementById('btn-AddShip').disabled= false;
        }
    }
